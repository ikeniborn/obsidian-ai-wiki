...
// eval/claude-probe/obsidian-stub.ts
var Platform = { isDesktopApp: true, isMobile: false };
...
function validateIclaudePath(p) {
  ...
  if (!(0, import_path_browserify.isAbsolute)(p)) throw new Error(`iclaudePath must be absolute: "${p}"`);
  if (p.split("/").includes("..")) throw new Error(`iclaudePath contains path traversal: "${p}"`);
}
async function probeClaudeBinary(iclaudePath) {
