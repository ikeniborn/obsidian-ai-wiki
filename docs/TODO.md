...
| domain-type-routing | done | n/a | n/a | n/a | OK | 2026-07-15 | 2026-07-15 | Directed bugfixes (no formal chain): (1) stop recreating !Wiki/_config on init; (2) server-enforced per-entity-type page routing (entity-routing.ts) + LLM classifier fallback + reject, no entities/ default; (3) preserve metadata.jsonl during force-reinit wipe; (4) isResuming keys on entity_types not analyzed_sources so bootstrap runs for a fresh domain (else zero types → routing rejects every page) |
