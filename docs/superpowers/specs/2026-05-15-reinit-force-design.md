...
review:
  ...
  findings:
    - id: F-001
      phase: structure
      severity: WARNING
      section: "## Non-goals (повтор)"
      section_hash: 0d8c4dba6c40a1a1
      text: "Дублирующийся раздел Non-goals (lines 28 и 185). Слияние выполнено: уникальный пункт «Декомпозиция runInitWithSources» добавлен в основной §Non-goals, секция-повтор удалена."
      verdict: fixed
