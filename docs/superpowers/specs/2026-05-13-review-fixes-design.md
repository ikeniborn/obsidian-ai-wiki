...
// стало
dropEl = (activeDocument as Document).body.createDiv({ cls: "ai-wiki-folder-dropdown" });
```

`createDiv` — расширение Obsidian на `HTMLElement`, работает через Obsidian-типизацию.
...
Все `clearInterval` → `clearTimeout`.
