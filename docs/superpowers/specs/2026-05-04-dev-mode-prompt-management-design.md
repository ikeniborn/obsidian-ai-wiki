...
2. Название поля в UI: "User prompt" (было "System prompt")
3. Описание: "Добавляется в конец системного промта каждой операции как раздел «Уточнение». По умолчанию пусто."
4. `injectSystemPrompt()` изменяет логику: вместо простой конкатенации добавляет именованный раздел:
   ```
   <существующий системный промт>

   ## Уточнение
   <userPrompt>
   ```
5. Если `userPrompt` пуст — раздел не добавляется (поведение без изменений)
