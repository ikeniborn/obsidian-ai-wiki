...
Two regressions surfaced after the prompt/template i18n work (commit `de2e79e`,
"translate all prompts/templates to English") and the move to settings-driven
output language:
...
- `src/utils/raw-frontmatter.ts` — extract shared `restoreSourceFrontmatter`.
...
- `lat check` passes; `lat.md/` updated if behavior docs reference the format flow.
