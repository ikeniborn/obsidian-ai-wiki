...
runFormat()
  ...
  → если missing.length > 0:
      callOnce(restoreParams)               # НОВЫЙ token-retry
      extractJsonObject() на новом ответе
      → missingTokensWithContext() повторно
      → если всё ещё missing:
          appendMissingLines(original, formatted, missing)
...
assistant: <первый formatted JSON-ответ>
...
| JSON-retry уже выполнен + token-retry нужен | Token-retry использует результат JSON-retry как `formatted` |
| Token-retry вернул невалидный JSON | Используем `formatted` из первого вызова + `appendMissingLines` |
...
- `format-utils.ts: appendMissingLines` — unit-тест: дедупликация, пустой context, уже есть HR в конце
