...
- **Fix** — действие из боковой панели после Lint: принимает инструкцию, передаёт модели, обновляет страницы
...
|---|---|---|
| User prompt | Добавляется в конец системного промпта каждой операции | пусто |
| Max tokens | Макс. токенов в ответе. Рекомендуется ≥ 4096. Показывается когда per-operation выключен и бэкенд native-agent | `4096` |
...
| Лог агента (JSONL) | Запись событий в `<vault>/!Logs/agent.jsonl` (только desktop) | выкл |

### Домены

Список созданных доменов с кнопками **Edit** / **Delete**. Карта доменов в `!Wiki/_domain.json`.

### Выбор бэкенда

| Параметр | Описание | По умолчанию |
|---|---|---|
| Backend | `claude-agent` или `native-agent` (desktop). На мобильном принудительно native-agent | `claude-agent` |
...
| Модель | Пресет (`opus`/`sonnet`/`haiku`) или явный ID (`claude-opus-4-7`). Показывается когда per-operation выключен | дефолт claude |
| Allowed tools | Список через запятую, передаётся в `--tools`. Пусто = без ограничения | `Read,Edit,Write,Glob,Grep` |
| Per-operation models | Тоггл. Когда включён — отдельная модель на каждую операцию (ingest/query/lint/init/format) | выкл |
| Per-operation: Model | Имя модели для конкретной операции | — |
...
| Контекстное окно (num_ctx) | Размер контекста (только Ollama). Пусто = дефолт модели | — |
| Temperature | `0.0`–`1.0`. Низкая (`0.1`–`0.3`) = точные факты | `0.2` |
| Per-operation models | Тоггл (только desktop). Когда включён — `model`/`maxTokens`/`temperature` на каждую операцию | выкл |
| Per-operation: Max tokens | Per-op макс. токенов. Дефолты: ingest/query `4096`, lint/init `8192`, format `32768` | — |
| Per-operation: Temperature | Per-op temperature (0–2) | `0.2` |
| Structured output retries | Ретраи при ошибке валидации схемы (0–3). Выше = надёжнее на слабых моделях ценой latency/токенов | `1` |

### Прокси (только native-agent)

| Параметр | Описание | По умолчанию |
|---|---|---|
| Use proxy | Пускать трафик native-agent через HTTP/HTTPS-прокси. На мобильном не поддерживается | выкл |
...
Прокси применяется только к native-agent. Claude Agent использует собственную конфигурацию.

### Graph

| Параметр | Описание | По умолчанию |
|---|---|---|
| BFS depth (graphDepth) | Query: число прыжков от seed-страниц. `0` = только seeds, максимум разумный `3` | `1` |
| Hub threshold (hubThreshold) | Lint: страницы с числом исходящих ссылок выше порога помечаются как хабы | `20` |

### Dev mode (только desktop)

| Параметр | Описание | По умолчанию |
|---|---|---|
| Dev mode | Включить dev-логгер и evaluator после каждой операции | выкл |
| Evaluator model | Имя модели для evaluator (тот же бэкенд) | — |
