...
AI Wiki запускает внешний процесс для работы бэкенда Claude CLI:

- **Что запускается:** абсолютный путь, указанный в Settings → Backend → «Path to Claude Code» (например, `/home/user/iclaude.sh`). Перед каждым запуском путь проверяется: он должен быть абсолютным и не содержать traversal-последовательностей.
- **Почему это необходимо:** бэкенд Claude Agent работает, вызывая `claude` / `iclaude.sh` как подпроцесс. Альтернативы `child_process.spawn` в данной архитектуре нет.
- **Права доступа:** подпроцесс наследует права вашего пользователя ОС — те же, что при ручном запуске Claude CLI в терминале.
- **Как проверить / изменить путь:** Settings → Backend Settings → «Path to Claude Code».
- **Согласие при первом запуске:** при первом запуске с выбранным бэкендом `claude-agent` появляется модальное окно с запросом явного подтверждения перед выполнением любой операции. Отозвать согласие можно, удалив `shellConsentGiven` из `data.json` плагина.

### Доступ к хранилищу

Плагин читает только папки, указанные как «Source paths» для каждого домена. Полный обход хранилища не выполняется.
