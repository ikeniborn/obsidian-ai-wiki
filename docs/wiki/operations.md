...
Quality is measured from real dev-mode runs labelled by hand, not a fixed gold set or an LLM judge. With `devMode.enabled`, **every** LLM operation run (`ingest`, `query`, `lint`, `lint-chat`, `chat`, `init`, `format`, `delete`) appends one record to `<pluginDir>/eval.jsonl` carrying harness telemetry (`llmErrors`, `ruleFirings`) and per-operation provenance; the user rates it 👍/👎 in the view, with the rating axes per operation drawn from the `OPERATION_AXES` registry (e.g. ingest → `page`+`links`, query → `answer`+`retrieval`). Any run is ratable — the just-finished one and every **historical** entry reopened from the history list — because the shared `renderResultFor` path rebuilds the rating UI per displayed `runId` (loading the persisted label), so ratings never leak across entries. The user can also attach one free-form `comment` per run (independent of the 👍/👎 label), persisted to the same record for reuse in optimization. See [[llm-pipeline#Dev-Mode Eval Record]].

`scripts/eval.ts` is a standalone report over that dataset (`npm run eval -- [--log <eval.jsonl>]`). It prints answer quality (query/chat), format quality split by vision on/off, recognition quality, and a per-`promptVersion` telemetry breakdown (👍-share, llmError counts, rule firings). It reads only `node:fs` + JSON — no `obsidian` shim. The same `eval.jsonl` feeds the `scripts/dspy` optimizer, which tunes prompts from the 👍/👎 labels with a baseline 👍-guard. Both readers (`scripts/eval.ts` and `scripts/dspy/lib/loader.py`) resolve the up/down signal from the per-axis `ratings` map — each operation's **primary axis** (`query→answer`, `chat→answer`, `format→formatting`, `ingest→page`, `init→coverage`, `lint→fix`, `lint-chat→fix`, `delete→rebuild`) — falling back to the legacy scalar `rating` for old records (`resolve_signal` / `resolveSignal`). The optimizer also folds each run's free-form `comment` into a per-operation "Human reviewer feedback" block (`scripts/dspy/lib/optimizer.py#build_feedback_block`) prepended to the seed prompt MIPROv2 rewrites — comments steer optimization as human opinion while the metric stays judge-free.
