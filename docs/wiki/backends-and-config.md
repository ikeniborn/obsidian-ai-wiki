...
- **Layer B — Reasoning language** (`resolveReasoningLang(reasoningLanguage, outputLanguage)`): injects a `## Reasoning language` directive into the system prompt (best-effort) from the shared, exported `reasoningDirective(lang)` (`src/phases/llm-utils.ts`). Explicit `ru|en|es` wins; `auto` chains to `resolveLang(outputLanguage)`; undefined defaults to `en` (models reason most reliably in English). The directive is imperative: reason exclusively in the chosen language, do not switch to match source notes/user input/quoted text, and apply the same rule to the `reasoning` field of any JSON output (the ingest/lint/eval drift source). The vision pre-step reuses the same `reasoningDirective` — see [[operations#Vision Pre-Step]].
- **Layer C — Generated content** (`resolveLang(outputLanguage)` via `langInstruction`): injects `## Language` into the system prompt to set the output language. The directive is imperative ("Write the entire response in <Language>. Do not switch to the source language."). Explicit `ru|en|es` wins; `auto`/undefined follows the Obsidian UI locale — **not** the source note language (deliberate change: `auto` no longer follows the source). Drives the localized wiki section headings via [[domain-model#Bundled Schemas]].
