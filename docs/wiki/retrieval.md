...
`pruneByRelevance` (`src/retrieval-prune.ts`) is pure: a graph page is kept iff its raw dense cosine ≥ `bar`, where `bar = loRef + bfsMinScoreRatio · (denseMax − loRef)` — a **spread-relative** threshold positioned a `bfsMinScoreRatio` fraction of the way up the domain's actual cosine dynamic range. `denseMax` is the best seed's cosine (top anchor); `loRef = robustLow(domainCosines, FLOOR_LO_PCT)` is a robust low percentile (`FLOOR_LO_PCT`, p5) of every domain page's raw cosine (lower anchor). This keeps biting when a model's absolute cosine range is compressed (e.g. deepseek ~0.40–0.59), where the previous absolute `bfsMinScoreRatio · denseMax` bar fell below the entire candidate cluster and pruned nothing until `bfsMinScoreRatio ≈ 0.95`. A page with no cosine is kept (cannot evaluate → no quality loss); `bfsMinScoreRatio ≤ 0` or a collapsed range (`denseMax − loRef < FLOOR_EPS`) keeps all. Seeds are never passed in — they are always kept.
...
The control is `nativeAgent.bfsMinScoreRatio` (`src/types.ts`, default `0.6`, range `0..1`, `0` = off) — now the bar's **position within `[loRef..denseMax]`**, not a fraction of `denseMax` — decoupled from `bfsFusion`. The `graph_stats` event carries `floorApplied`, `floorRef` (the `denseMax` top anchor), `floorLoRef` (the lower anchor used), `floorBar` (the computed threshold), `prunedCount`, `floorSkippedReason` (including `range-collapsed`), and `expandedDense` (the pre-prune raw cosine per graph candidate — the floor's actual decision input, for tuning) for the progress trace and `agent.jsonl`; `eval_meta.retrievalConfig` records the ratio. The `eval/graph-floor` harness — `run.ts` (headless live capture against the embedding endpoint) plus `analyze.ts` (deterministic ratio sweep) — replays gold queries to calibrate `bfsMinScoreRatio` and `FLOOR_LO_PCT` against token reduction and gold-page recall. See [[operations#Query]].
