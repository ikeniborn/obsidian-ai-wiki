...
When `opts.outputLanguage` is set, appends a `## Language` directive (from `langInstruction`, layer C). Always appends a `## Reasoning language` directive resolved from `opts.reasoningLanguage` and `opts.outputLanguage` (layer B, defaults to English). See [[backends-and-config#Three-Layer Resolution]]. Applies model params: `temperature`, `maxTokens`, `topP`, `response_format`, `thinkingBudgetTokens`. Thinking mode removes `response_format`, `temperature`, `top_p`.
