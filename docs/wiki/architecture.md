...
Persists domain mutations from events via `DomainStore`. `logEvent` writes JSONL to `<pluginDir>/agent.jsonl` (the plugin dir via `pluginDir()`, NOT the synced wiki tree) when `agentLogEnabled` — so logs land live without an Obsidian restart and never pollute the wiki output. Model reasoning is buffered (`_reasoningBuf`) across `assistant_text`+`isReasoning` deltas and flushed as one consolidated `{event:{kind:"reasoning"}}` record per LLM call when the next non-`assistant_text` event arrives, stamped with the current `_llmCallIndex`; non-reasoning `assistant_text` chatter stays dropped (the final answer is captured by the `result` event). The buffer resets per operation in `dispatch`.
