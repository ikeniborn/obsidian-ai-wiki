...
Версия: **0.1.68** | Язык: TypeScript | Runtime: Electron (Obsidian Desktop) + WebView (Obsidian Mobile)
...
- **Missing-tokens UI (v0.1.68):** warning рендерится как `<details>` с раскрываемым `<ul>` — пользователь видит конкретный список «потерянных» токенов и оценивает шум (переформулировки section-меток, переписанные комментарии в коде) vs реальную потерю фактов. CSS: `max-height: 160px; overflow-y: auto`, monospace 11px.
...
- **stripCodeFence anchored (v0.1.68):** regex привязан к началу/концу ответа (`^\s*\`\`\`...\`\`\`\s*$`). Прежний жадный `/\`\`\`(?:json)?[\s\S]*?\`\`\`/` ловил ПЕРВЫЙ внутренний markdown-fence из поля `formatted` (```sql, ```bash, ```yaml) — возвращал чужой блок без `{`, парсер падал → ложный «JSON невалиден → retry → final error». Воспроизводилось у любой модели, чей JSON-ответ не обёрнут во внешний ```json fence (sonnet, gpt). Haiku маскировала баг собственным внешним fence. Тесты: `tests/phases/format-utils.test.ts` (regression).
- **Result-text без JSON-дампа (v0.1.67):** при ошибке (`finish_reason === "length"`, невалидный JSON после retry) `runFormat` эмитит `result` с `text: ""` вместо сырого JSON-сурового тела ответа в Result-блок UI — раньше пользователь видел в правой панели стенограмму JSON со спецсимволами. Текст ошибки идёт через отдельное событие, Result-блок остаётся пустым.
- **Per-op maxTokens (v0.1.66):** удалён из `claude-agent` целиком — claude CLI не принимает `--max-tokens`, лимит задаётся через env `CLAUDE_CODE_MAX_OUTPUT_TOKENS` в `iclaude.sh`. `loadSettings()` стрипает legacy `claudeAgent.operations.*.maxTokens` из `data.json`. У `native-agent` поле сохранено (default `format=32768` после миграции v0.1.65).
...
### 7b. Mobile UI Gating in View (v0.1.68, in progress)
`LlmWikiView` рендерится одним bundle на desktop и mobile, но конкретные секции боковой панели скрываются на mobile. Поля контролов помечены опциональными (`domainSelect?`, `initBtn?`, `ingestBtn?`, `lintBtn?`, `formatBtn?`) — на mobile они не создаются, и весь код, обращающийся к ним, использует optional chaining (`this.domainSelect?.value`). Секции «Создание домена» и «Наполнение / Актуализация» обёрнуты `if (!Platform.isMobile) { ... }`. Гварды добавлены в `refreshDomains()`, `openAddDomain()`, `submitQuery()`, `setRunning()` — методы пропускают манипуляции над несозданными элементами вместо падения с `Cannot read properties of undefined`. На mobile UI остаётся только Query-блок, чат и история — соответствует ограничению `dispatch()` (только query/query-save).
