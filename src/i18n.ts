...
const en = {
  ...
  settings: {
    ...
    contextWindowTokens_desc: "How many tokens the model holds in one request. Leave empty for automatic — the window is read from the backend. Set it only when your backend does not report one: the symptom is a fallback window of 8192 in the agent log, dropped schema blocks, and requests truncated against a 4096-token limit. Every budget is then derived from your number. Applies to all native models.",
    ...
    llmIdleTimeout_desc: "Seconds of LLM silence before aborting the attempt. 0 = disabled; maximum 2,146,999. Default 300. ↑ more patience for slow models · ↓ catches stalls sooner.",
...
const ru: I18n = {
  ...
  settings: {
    ...
    contextWindowTokens_desc: "Сколько токенов модель вмещает в один запрос. Пусто — автоматически: окно читается у бэкенда. Задавайте значение, только если бэкенд его не сообщает: признаки — запасное окно 8192 в agent-логе, выброшенные блоки схемы и запросы, обрезанные лимитом в 4096 токенов. Тогда все бюджеты выводятся из вашего числа. Применяется ко всем нативным моделям.",
    ...
    llmIdleTimeout_desc: "Секунд тишины LLM до прерывания попытки. 0 = отключено; максимум 2 146 999. По умолч. 300. ↑ терпеливее к медленным моделям · ↓ раньше ловит зависания.",
...
const es: I18n = {
  ...
  settings: {
    ...
    llmIdleTimeout_desc: "Segundos de silencio del LLM antes de abortar el intento. 0 = desactivado; máximo 2.146.999. Por defecto 300. ↑ más paciencia con modelos lentos · ↓ detecta bloqueos antes.",
