...
const en = {
  ...
  settings: {
    ...
    autoBudgetNotice_body: "Native input and output budgets are now derived automatically from the model's context window. You have a stored budget override from before this change. Switch to automatic, or keep your stored values? You can still set an override later under Advanced.",
...
const ru: I18n = {
  ...
  settings: {
    ...
    autoBudgetNotice_body: "Бюджеты ввода и вывода для нативного бэкенда теперь автоматически выводятся из контекстного окна модели. У вас сохранено переопределение бюджета, заданное до этого изменения. Переключиться на автоматический режим или оставить сохранённые значения? Переопределение всегда можно задать позже в разделе «Дополнительно».",
...
const es: I18n = {
  ...
  settings: {
    ...
    autoBudgetNotice_body: "Los presupuestos nativos de entrada y salida ahora se derivan automáticamente de la ventana de contexto del modelo. Tiene guardada una anulación de presupuesto de antes de este cambio. ¿Cambiar a automático o conservar sus valores guardados? Aún puede definir una anulación más adelante en Avanzado.",
