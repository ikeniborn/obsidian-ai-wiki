...
Creates the `DomainEntry` and per-domain `_config`. Wiki/format schema conventions are bundled into the plugin (not written to the vault) — see [[domain#Bundled Schemas]]. See [[src/phases/init.ts]], [[llm-pipeline#parseWithRetry]].
...
When `vision.enabled` and `vision.model` are set, a pre-step analyzes embedded images, PDFs, and Excalidraw files. Excalidraw embeds are rendered to PNG by the host `obsidian-excalidraw-plugin` via `ExcalidrawAutomate` (no `@excalidraw/utils` dependency); when the host plugin is absent or on mobile, the embed is skipped. Each attachment emits `tool_use`/`tool_result` events for sidebar progress. Descriptions are inserted into the formatted output only — the source file is never modified. Image and PDF diagram embeds work in two stages: the model reads the drawing literally as a silent internal step (nodes, labels, connections), then emits a structured, logical description of what the scheme means for a reader — purpose, components/actors, how the flow connects them, synthesized rather than a verbatim element-by-element dump; they then also recreate the structure: a `mermaid` block for flow/architecture schemes or a markdown table for grid/matrix schemes (image diagrams via the shared structure rules, PDF via [[src/phases/attachment-analyzer.ts#pdfSystem]]). Excalidraw uses a dedicated prompt ([[src/phases/attachment-analyzer.ts#excalidrawSystem]]) that always treats the render as a scheme but works in two stages: it reads the canvas literally as a silent internal step (nodes, labels, connections), then emits only a structured, logical description of what the scheme MEANS in business terms — its purpose, components/actors, and how the flow connects them — synthesized rather than a verbatim element-by-element dump, prose and lists only, no `mermaid`. Language is controlled by `vision.language` (`auto`/`ru`/`en`/`es`). Vision events are logged to `_agent.jsonl`. See [[src/phases/attachment-analyzer.ts]].

## Retrieval Eval Harness

Standalone CLI (`scripts/eval.ts`) measuring retrieval quality — Recall@k (k=3,5,8) and MRR — against a fixed gold set, per layer (seed, union) and config (dense, jaccard). Distinct from the answer-quality evaluator ([[src/phases/evaluator.ts]]), which scores the LLM answer.
