...
When `vision.enabled` and `vision.model` are set, a pre-step analyzes embedded images, PDFs, and Excalidraw files. Excalidraw embeds are rendered to PNG by the host `obsidian-excalidraw-plugin` via `ExcalidrawAutomate` (no `@excalidraw/utils` dependency); when the host plugin is absent or on mobile, the embed is skipped. Each attachment emits `tool_use`/`tool_result` events for sidebar progress. Descriptions are inserted into the formatted output only — the source file is never modified. Diagram embeds yield a detailed, verbatim description of everything drawn — every node/label transcribed exactly and every connection with its direction and edge labels — not a brief summary. Image and PDF diagrams then also recreate the structure: a `mermaid` block for flow/architecture schemes or a markdown table for grid/matrix schemes (image diagrams via the shared structure rules, PDF via [[src/phases/attachment-analyzer.ts#pdfSystem]]). Excalidraw uses a dedicated prompt ([[src/phases/attachment-analyzer.ts#excalidrawSystem]]) that always treats the render as a scheme and emits the verbatim description only — no `mermaid`. Language is controlled by `vision.language` (`auto`/`ru`/`en`/`es`). Vision events are logged to `_agent.jsonl`. See [[src/phases/attachment-analyzer.ts]].
