...
Release history is immutable: `versions.json["0.3.5"]` stays `"1.7.2"`. Current `0.3.13` package, lockfile, source/root/distribution manifest, and compatibility metadata are synchronized; `0.3.13` maps to `1.13.0` and every current manifest declares the same minimum app version.
