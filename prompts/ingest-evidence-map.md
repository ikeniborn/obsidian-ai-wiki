...
- Every packet has a unique CHUNK-LOCAL id (for example `chunk-local-p1`; the server namespaces it with the supplied chunkId), the supplied chunkId, a normalized lowercase entityKey, optional configured entityType only when configured entity types are supplied, one or more atomic facts, one or more exact one-based source ranges RELATIVE TO THE PROVIDED CHUNK, links copied from the source, and a sourceAnchor.
