...
| `tags` | YAML-список: `[category/subcategory, domain/topic]`. Иерархия через `/`, строчные, без пробелов, без `#`. Только теги из tag-hierarchy.json. Obsidian распознаёт ключ `tags` автоматически — тип явно не указывать. |
