...
### Fixes
- fix(vision): size image and PDF analysis from the vision model's own context window instead of the chat model's. Nothing resolved a context record for `vision.model`, so PDF page batches were packed against the text model's budget — on a 128k chat model that is ~28 pages in one call to a possibly 8k-window vision model, which the provider rejects and Format reports as "Vision skipped" with the attachment silently missing from the index. Vision now resolves and calibrates its own record and receives its own input, output and calibration values.
