...
- fix(llm): price prompt characters by class instead of one flat rate for everything but Cyrillic. The flat 4.2 characters-per-token was a prose rate, so a prompt of shell commands, config files, iptables rules and JSON was counted as if it were prose: on a live Linux-notes vault the estimate tracked the provider on 3.5k-token prompts and fell 22% short on a 17.6k-token one, and because the shortfall is a slope, not a constant, no calibration factor — a single multiplier — could remove it. Letters and the spaces that bind them into words, Cyrillic, and digit/symbol runs now carry their own rates, fitted against the provider's own input counts for 82 recorded live prompts. Across those prompts the size-dependence is gone (correlation with prompt size falls from -0.87 to -0.03) and every one lands inside the 15% band, biased upward. Chunk boundaries move with the rates, so an existing domain needs `Init --force` to re-chunk.
