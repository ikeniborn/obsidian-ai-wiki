...
- feat(native-agent): give every configured model its own "Model context window" field — the chat model, each per-operation model, and the vision model — instead of one global number forced onto all of them. A saved single value is migrated onto every Native Agent chat model it used to cover, so nothing is lost on upgrade; clearing one model's window leaves the others alone, and two roles naming the same model share one window because the context record is cached per model.

### Fixes
- fix(vision): size image and PDF analysis from the vision model's own context window instead of the chat model's. Nothing resolved a context record for `vision.model`, so PDF page batches were packed against the text model's budget — on a 128k chat model that is ~28 pages in one call to a possibly 8k-window vision model, which the provider rejects and Format reports as "Vision skipped" with the attachment silently missing from the index. Vision now resolves its own record and receives its own input, output and calibration values. This applies only when a window for the vision model is actually known — advertised by the backend, typed into its "Model context window" field, or learned from a provider rejection. When nothing is known, vision keeps being sized from the Format operation's budget exactly as before, because the 8192-token fallback is not a measurement of the vision model and budgeting from it would leave less room than a single image costs.
- fix(vision): explain a Vision attachment skipped for size. Both size refusals are decided before the request is sent, so the provider's answer explains nothing and the budget telemetry that would is agent-log-only. The "Vision skipped" warning now names the vision model, the window it was measured against, whether that window was configured, discovered, learned or a fallback, and the setting that changes it.
