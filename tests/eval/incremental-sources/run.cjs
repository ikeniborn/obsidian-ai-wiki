...
// tests/eval/incremental-sources/run.ts
var pass = 0;
var fail = 0;
var failures = [];
...
check(
  "no-frontmatter passthrough (trailing trimmed)",
  sourceBodyForHash("plain body\n\n") === "plain body"
);
check(
  "hashSource has fnv1a prefix + 8 hex",
...
check(
  "frontmatter-only edit \u2192 SAME hash",
  hashSource("---\nwiki_updated: 2026-06-01\n---\nBODY") === hashSource('---\nwiki_updated: 2026-06-25\nwiki_articles:\n  - "[[p]]"\n---\nBODY')
);
check(
  "body edit \u2192 DIFFERENT hash",
  hashSource("---\nx: 1\n---\nBODY one") !== hashSource("---\nx: 1\n---\nBODY two")
);
...
check("5 strict subset", (() => {
  const r = computeChangedSources({
    sourceFiles: [
      { path: "s/a.md", hash: "fnv1a:0000aaaa" },
      // matches stored → skip
      { path: "s/b.md", hash: "fnv1a:0000ffff" }
      // differs → changed
    ],
...
check("7 idempotent (already v3) \u2192 no change", (() => {
  const domains = [{
    ...
    name: "D",
    wiki_folder: "d",
    analyzed_sources: { "x.md": "fnv1a:0000aaaa" },
    analyzed_sources_v2: true,
...
check(
  "pp double-quoted stem \u2192 bare stem",
  JSON.stringify(parsePageSources('---\nresource:\n  - "alpha"\n---\nx')) === JSON.stringify(["alpha"])
);
check(
  "pp unquoted stem \u2192 bare stem",
  JSON.stringify(parsePageSources("---\nresource:\n  - alpha\n---\nx")) === JSON.stringify(["alpha"])
);
check(
  "pp multiple entries",
...
check(
  'pp flow-form resource: ["stem"] \u2192 bare stem (ingest prompt shape)',
  JSON.stringify(parsePageSources('---\nresource: ["alpha"]\n---\nx')) === JSON.stringify(["alpha"])
);
check(
  "pp flow-form multiple entries",
  JSON.stringify(parsePageSources('---\nresource: ["a", "b"]\n---\nx')) === JSON.stringify(["a", "b"])
);
check(
  "pp block-form single-quoted stem (equivalence check)",
  JSON.stringify(parsePageSources("---\nresource:\n  - 'alpha'\n---\nx")) === JSON.stringify(["alpha"])
);
console.log(`
========================================`);
