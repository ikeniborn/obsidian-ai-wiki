...
async function logEvent(adapter: ReturnType<typeof createMockAdapter>, line: string): Promise<void> {
  ...
  if (!(await adapter.exists("!Wiki/_config"))) await adapter.mkdir("!Wiki/_config");
  const path = "!Wiki/_config/_agent.jsonl";
...
describe("logEvent — vault adapter writer", () => {
  it("creates !Wiki/_config and writes _agent.jsonl on first event", async () => {
    const a = createMockAdapter();
    await logEvent(a, '{"a":1}\n');
    expect(a.dirs.has("!Wiki")).toBe(true);
    expect(a.dirs.has("!Wiki/_config")).toBe(true);
    expect(a.files.get("!Wiki/_config/_agent.jsonl")).toBe('{"a":1}\n');
  });
