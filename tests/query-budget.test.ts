...
test("packQueryChunks keeps the exact current question and complete highest-score chunks", () => {
  ...
  // (task-3 prompt-budget-automation): 714 (3_000 / 4.2) still excludes
  // some of the 20 fixture chunks.
...
test("packQueryChunks rejects an oversized current question instead of truncating it", () => {
  ...
  // Rescaled from a byte-era budget of 2_000 for the token estimator
  // (task-3 prompt-budget-automation): 476 (2_000 / 4.2) is still smaller
  // than the oversized 5_000-character question's own token estimate.
  assert.throws(() => packQueryChunks({
    question,
    systemPrompt: "contract",
    chunks: [],
    inputBudgetTokens: 476,
    opts: {},
  }), (error) => {
